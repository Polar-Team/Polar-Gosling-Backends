...
class S3ArtifactCache:
    ...
    async def cache_provider_plugin(
        ...
        """
        s3_key = f"terraform-plugins/{provider_source}/{provider_version}/{os.path.basename(plugin_path)}"
        
        self.info(f"Caching provider plugin {provider_name} v{provider_version} to S3: {s3_key}")
    ...
    async def get_cached_provider_plugin(
        ...
        """
        s3_key = f"terraform-plugins/{provider_source}/{provider_version}/{plugin_filename}"
    ...
    async def get_cached_terraform_dir(
        ...
        async with await self._get_s3_client() as s3:
            try:
                ...
                self.info(f"Successfully retrieved cached .terraform directory: {s3_prefix}")
                return True
    ...
    async def cache_lock_file(
        ...
        """
        s3_key = f"terraform-plugins/lock.json" if lock_type == "provider" else f"modules/lock.json"
    ...
    async def invalidate_cache(
        ...
        identifier: Optional[str] = None,  # provider_source/version, module_name/version, or egg_name
    ) -> None:
        """
        ...
        """
        self.info(f"Invalidating {cache_type} cache" + (f" for {identifier}" if identifier else ""))
        ...
        async with await self._get_s3_client() as s3:
            try:
                if cache_type == "all":
                    ...
                    prefixes = [f"terraform-plugins/{identifier}/"] if identifier else ["terraform-plugins/"]
                elif cache_type == "module":
                    prefixes = [f"modules/{identifier}/"] if identifier else ["modules/"]
                elif cache_type == "terraform":
                    prefixes = [f".terraform/{identifier}/"] if identifier else [".terraform/"]
                else:
    ...
    async def verify_cache_integrity(
        ...
        async with await self._get_s3_client() as s3:
            try:
                ...
                self.warning(f"No checksum in metadata for {s3_key}, downloading to verify")
                import tempfile
