...
def idle_apex_runner_strategy(draw, egg_name: str = "test-egg", idle_minutes: int = 35):
    ...
    last_heartbeat = datetime.now(UTC) - timedelta(minutes=idle_minutes)
    ...
    return Runner(
        ...
        updated_at=datetime.now(UTC) - timedelta(minutes=idle_minutes),
...
async def test_auto_demote_idle_runners():
    ...
    idle_runners = [
        Runner(
            ...
            last_heartbeat=datetime.now(UTC) - timedelta(minutes=35),
            failure_count=0,
            metadata={},
        )
        for i in range(2)
